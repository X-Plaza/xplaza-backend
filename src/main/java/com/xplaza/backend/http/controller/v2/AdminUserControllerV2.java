...
/**
 * V2 Admin User Controller - Clean REST API design.
 */
@RestController
...
@Validated
@Slf4j
@Tag(name = "Admin User Management V2", description = "V2 APIs for managing admin users")
public class AdminUserControllerV2 {
  ...
  public ResponseEntity<ApiResponseV2<List<AdminUserResponse>>> getAdminUsers(
  ...
  public ResponseEntity<ApiResponseV2<AdminUserResponse>> getAdminUser(@PathVariable @Positive Long id) {
  ...
  public ResponseEntity<ApiResponseV2<Void>> createAdminUser(@RequestBody @Valid AdminUserRequest request) {
    AdminUser entity = adminUserMapper.toEntity(request);
    adminUserService.addAdminUser(entity);
    return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponseV2.ok("Admin user created successfully"));
  }

  @PutMapping("/{id}")
  @Operation(summary = "Update admin user", description = "Update an existing admin user")
  public ResponseEntity<ApiResponseV2<AdminUserResponse>> updateAdminUser(
  ...
  public ResponseEntity<ApiResponseV2<Void>> deleteAdminUser(@PathVariable @Positive Long id) {
    adminUserService.deleteAdminUser(id);
    return ResponseEntity.ok(ApiResponseV2.ok("Admin user deleted successfully"));
  }
  ...
  public ResponseEntity<ApiResponseV2<Void>> changePassword(
    ...
    if (!isValidUser) {
      return ResponseEntity.status(HttpStatus.FORBIDDEN)
          .body(ApiResponseV2.error("INVALID_CREDENTIALS", "Old password does not match"));
    }
    ...
    try {
      ...
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
          .body(ApiResponseV2.error("INTERNAL_ERROR", "Failed to change password"));
    }
  }
