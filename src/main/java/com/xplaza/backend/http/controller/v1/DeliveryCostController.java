...
/**
 * Delivery Cost Controller - Clean REST API design.
 */
@RestController
...
public class DeliveryCostController {
  ...
  public ResponseEntity<ApiResponse<List<DeliveryCostResponse>>> getDeliveryCosts(
  ...
  public ResponseEntity<ApiResponse<DeliveryCostResponse>> getDeliveryCost(@PathVariable @Positive Long id) {
  ...
  public ResponseEntity<ApiResponse<Void>> createDeliveryCost(@RequestBody @Valid DeliveryCostRequest request) {
    DeliveryCost entity = deliveryCostMapper.toEntity(request);
    deliveryCostService.addDeliveryCost(entity);
    return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.ok("Delivery cost created successfully"));
  }

  @PutMapping("/{id}")
  @Operation(summary = "Update delivery cost", description = "Update an existing delivery cost")
  public ResponseEntity<ApiResponse<Void>> updateDeliveryCost(
    ...
    return ResponseEntity.ok(ApiResponse.ok("Delivery cost updated successfully"));
  }

  @DeleteMapping("/{id}")
  @Operation(summary = "Delete delivery cost", description = "Delete a delivery cost")
  public ResponseEntity<ApiResponse<Void>> deleteDeliveryCost(@PathVariable @Positive Long id) {
    String deliverySlab = deliveryCostService.getDeliverySlabRangeNameByID(id);
    deliveryCostService.deleteDeliveryCost(id);
    return ResponseEntity.ok(ApiResponse.ok("Delivery cost of " + deliverySlab + " order range has been deleted"));
  }
}
